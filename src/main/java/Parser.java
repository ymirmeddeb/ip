        ...
        switch (type) {
            ...
            case "deadline":
                ...
                if (deadlineParts.length < 2) {
                    throw new JoeException("Invalid deadline format. Please use the format: deadline [description] /by yyyy-MM-dd HHmm");
                }
                try {
                    // Attempt to parse the date string
                    LocalDateTime by = LocalDateTime.parse(deadlineParts[1], DateTimeFormatter.ofPattern("yyyy-MM-dd HHmm"));
                    return new Deadline(deadlineParts[0], by);  // Use the original string, assuming your Deadline constructor handles the conversion.
